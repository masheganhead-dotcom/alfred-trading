...
function buildPsych(userCtx, dayAnalysis) {
  ...
  const tips = [];
  const mbti = userCtx.user.mbti || userCtx.user.mbti_hint || "";
  const isIntrovert = b5.E < 50 || mbti.startsWith("I");
  const isIntuitive = mbti.includes("N");
  const isJudger = mbti.includes("J");

  // 신경증 처방 (점수와 무관 — INFJ는 항상 챙겨야)
  if (b5.N >= 60) {
    if (dayAnalysis.score < 50) tips.push(`신경증 ${b5.N}점 — 안 좋은 날엔 더 예민. 오후 카페·지하철 피하기. 노이즈 캔슬링 켜.`);
    else tips.push(`신경증 ${b5.N}점 — 좋은 날에도 자극 누적되면 밤에 폭발. 저녁은 혼자.`);
  }

  // 내향 처방 (INFJ 핵심)
  if (isIntrovert) {
    if (dayAnalysis.score < 50) tips.push(`내향(E ${b5.E}) — 오늘 약속 0~1개로 제한. 사람 만나면 더 소진. 집·작업실 단독.`);
    else tips.push(`내향(E ${b5.E}) — 좋은 날이라도 약속 1~2개까지. 만남 후 1시간 혼자 회복 시간 필수.`);
  }

  // 친화성 낮음 — 공통
  if (b5.A <= 30) tips.push(`친화성 ${b5.A}점 (낮음) — 협업 때 "그냥 내 페이스로 갈게" 한 마디 미리. 무리해서 맞추지 X.`);

  // 직관 + 판단 (INFJ 일처리)
  if (isIntuitive && isJudger) {
    tips.push(`${mbti} — 직관·완결형. "왜 하는지" 의미부터 잡고 시작. 디테일 욕심은 끝나고. 시작 전 1분 명상.`);
  } else if (mbti) {
    tips.push(`${mbti} — 자기 스타일대로.`);
  }
...
export function generateDailyStory({ mySaju, date = new Date(), userName = "성훈", userContext = null }) {
  if (!userContext) {
    userContext = { user: { name: userName, day_stem: mySaju.dayMaster.stem, day_branch: mySaju.pillars.day.branch,
                            ...
                            big5: { O:50, C:50, E:50, A:50, N:50 }, mbti: "INFJ" },
